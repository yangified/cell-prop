...
%
% >> [mu,sigma] = CompareMaps('Measurements_ABCD','FakePredictionData_ABCD');
%
...
% yourself (surname first), i.e. 'Smith_John.mat'.  Since you don't know
% where the measured points are, you must make a 'meas' for each map E, F,
% G, and H that has a prediction for every pixel.
%
% You may also use this same file to test your modeling skills on the sites that have available
...
for i = 1:length(Names),
    ...
    I = Measurement.data ~= 0;
    if any(any(I))
       Prediction = getfield(Y,Names{i});
        error = double(Prediction.data(I)) - double(Measurement.data(I));
        mu = [ mu; sum(error)/length(error) ];
        sigma = [ sigma; (sum((error-mu(i)).^2)/length(error))^.5 ];
    
        Title = Names{i}; Title( find(Title=='_') ) = ' ';
        figure(i); imagesc(Prediction.data); axis equal; axis xy; colorbar; title(['Prediction for ' Title]);
